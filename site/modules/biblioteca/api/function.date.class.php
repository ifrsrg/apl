...
class WDate {
	...
	static function diaExtenso($data,$feira=false){
		...
		switch (date("w",mktime(0,0,0,$mes,$dia,$ano))) {
			...
			case 2: return "Ter�a"  .$comp; break;
    ...
    static function formatByStr($str, $dataOuTimestamp) {
		$dataOuTimestampTest = str_replace("-", "", $dataOuTimestamp);
		$dataOuTimestampTest = str_replace("/", "", $dataOuTimestampTest);
		$dataOuTimestampTest = str_replace("0", "", $dataOuTimestampTest);
		if ($dataOuTimestampTest) {
			...
			if ($hora) {
				$vHora = explode(":", $hora);
				$data .= " ".substr($hora,0,5);
			} else {
				$vHora = array(0,0,0);
			}
			return date($str, mktime($vHora[0],$vHora[1],$vHora[2],$vData[1],$vData[2],$vData[0]));
